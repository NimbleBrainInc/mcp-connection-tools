...
BUNDLE_NAME = mcp-connection-tools
VERSION ?= 0.1.0

.PHONY: help install dev-install format format-check lint lint-fix test test-cov test-e2e clean check all bundle run run-http bump
...
test-cov: ## Run tests with coverage
	uv run pytest tests/ -v --cov=src/mcp_connection_tools --cov-report=term-missing
...
run: ## Run the MCP server in stdio mode
	uv run python -m mcp_connection_tools.server

run-http: ## Run HTTP server with uvicorn
	uv run uvicorn mcp_connection_tools.server:app --host 0.0.0.0 --port 8080
...
bump: ## Bump version across all files (usage: make bump VERSION=0.2.0)
	...
	@sed -i '' 's/^__version__ = ".*"/__version__ = "$(VERSION)"/' src/mcp_connection_tools/__init__.py
	@echo "Updated:"
	@echo "  manifest.json:                           $$(jq -r .version manifest.json)"
	@echo "  server.json:                             $$(jq -r .version server.json)"
	@echo "  pyproject.toml:                          $$(grep '^version' pyproject.toml)"
	@echo "  src/mcp_connection_tools/__init__.py:    $$(grep '__version__' src/mcp_connection_tools/__init__.py)"
